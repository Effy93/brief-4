...
class ArticleRepository {
    async readAll() {
        const [rows] = await databaseClient.query<Rows>("SELECT article.*, category.label AS category_label, category.id AS category_id FROM article LEFT JOIN article_category ON article.id = article_category.article_id LEFT JOIN category ON article_category.category_id = category.id");
    ...
    async readRandomOne() {
        const [rows] = await databaseClient.query<Rows>("SELECT * FROM article ORDER BY RAND() LIMIT 1");
        return rows[0] as Article;
    }

    async create(article: Article) {
        const [result] = await databaseClient.query<Result>("INSERT into article (notion, content) VALUES (?, ?)", 
